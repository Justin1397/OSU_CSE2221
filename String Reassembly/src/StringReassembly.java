...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //get the part of str1 which not includes the overlap part
        String str1Part = str1.substring(0, str1.length() - overlap);
        //add it to str2 to get the result
        String combine = str1Part + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        if (strSet.size() == 0) {
            strSet.add(str);
            //if it is not empty, check if it is substring of any element
        } else if (!strSet.contains(str)) {
            boolean isSub = false;
            for (String strIn : strSet) {
                //test if it is substring of any, if so, don't add it
                if (strIn.indexOf(str) >= 0) {
                    isSub = true;
                }
            }
            //if it is not a substring then keep processing
            if (!isSub) {
                //add it to set
                strSet.add(str);
                Set<String> temp = strSet.newInstance();
                temp.transferFrom(strSet);
                //go over to test if any element is substring of str
                while (temp.size() > 0) {
                    String strMove = temp.removeAny();
                    //add the str
                    if (strMove.equals(str)) {
                        strSet.add(strMove);
                        //if is not a substring of str, add back
                    } else if (str.indexOf(strMove) < 0) {
                        strSet.add(strMove);
                    }
                }
            }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //test it if has next line
        while (!input.atEOS()) {
            //if so, read next line
            String lines = input.nextLine();
            //add the line to set and avoid the substring condition
            addToSetAvoidingSubstrings(result, lines);
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        String newText = "";
        //go over each char in text
        for (int i = 0; i < text.length(); i++) {
            char newChar = text.charAt(i);
            //if it is '~', replace it as '\n'
            if (newChar == '~') {
                newText += "\n";
            } else {
                //if not, add char back
                newText += newChar;
            }
