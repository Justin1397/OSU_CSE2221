...
public final class CryptoUtilities {
     ...
     */
    private static final int NUM_CANDIDATE = 50;
    /**
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        //check if it is zero
        if (!m.isZero()) {
            //if is not, repeat with GCD(m, n mod m)
    ...
    public static boolean isEven(NaturalNumber n) {
        //create the variable and NaturalNumber needed
        boolean check = false;
        NaturalNumber two = new NaturalNumber2(2);
        //get the remainder when n/2
        NaturalNumber remain = n.divide(two);
        //check if it has remainder
        if (remain.isZero()) {
            //if yes, it is a even number
            check = true;
        }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        //create the NaturalNumbers needed
        ...
        if (!pCopy.isZero()) {
            if (isEven(pCopy)) {
                ...
                //if it is odd, decrease it by one
                pCopy.decrement();
                NaturalNumber nCopy = new NaturalNumber2(n);
                powerMod(n, pCopy, m);
                n.multiply(nCopy);
            ...
            //calculate the  #n^(p) mod m and give n the value
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        n.increment();
        //create the variable and NaturalNumbers needed
        boolean check = false;
        NaturalNumber oldW = new NaturalNumber2(w);
        NaturalNumber two = new NaturalNumber2(2);
        NaturalNumber one = new NaturalNumber2(1);
        //calculate the w^2 mod n
        powerMod(w, two, n);
        //test if the w^2 mod n is equal to one
        if (w.equals(one)) {
            check = true;
        }
        //restore the w
        ...
        //calculate the w^(n-1) mod n
        n.decrement();
        powerMod(w, n, oldN);
        //test if the w^(n-1) mod n is not equal to one
        if (!w.equals(one)) {
            check = true;
        }
        //restore n
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        NaturalNumber two = new NaturalNumber2(2);
        NaturalNumber one = new NaturalNumber2(1);
        //to generate several witness candidates
        for (int i = 0; i < NUM_CANDIDATE; i++) {
            NaturalNumber randomN = randomNumber(n);
            //test if random number is obey the requires of isWitnessToCompositeness
            if (n.compareTo(two) > 0 && randomN.compareTo(one) > 0) {
                n.decrement();
                if (randomN.compareTo(n) < 0) {
                    n.increment();
                    //test if the random number is composite
                    if (isWitnessToCompositeness(randomN, n)) {
                        check = false;
                    }
                    n.decrement();
                }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        //use isPrime2 to check if it is a prime
        if (!isPrime2(n)) {
            //if not, increase it with one and test again until it is prime
            if (isEven(n)) {
                n.increment();
            } else {
                n.increment();
